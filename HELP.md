...
The application no longer contains a hardcoded OpenWeather key.  
Before running anything that talks to the real API, export a key into the runtime environment:

```bash
export WEATHER_API_KEY="<your-new-openweather-key>"
# or for one-off runs
WEATHER_API_KEY="<your-new-openweather-key>" ./mvnw spring-boot:run
```

Make sure to rotate the old leaked key in the OpenWeather dashboard so it cannot be used anymore.
